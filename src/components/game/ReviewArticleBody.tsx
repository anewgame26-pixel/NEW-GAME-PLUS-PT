...
interface ReviewArticleBodyProps {
  article: ReviewArticle;
  /** Mostra o cabeçalho "Review" + título — desliga dentro do hub do jogo, onde o título já aparece no topo da página. */
  showHeading?: boolean;
...
const SECTIONS: { field: keyof ReviewArticle & string; label: string }[] = [
...
function notaColor(nota: number) {
  if (nota >= 8) return "border-accent/40 bg-accent/10 text-accent";
  if (nota >= 5) return "border-gold/40 bg-gold/10 text-gold";
  return "border-primary/40 bg-primary/10 text-primary-light";
}

export function ReviewArticleBody({
  article,
  showHeading = true,
  extraSlot,
  author,
}: ReviewArticleBodyProps) {
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-3xl px-4 py-8 lg:px-8">
        ...
        {article.nota !== null && (
          <div className={`mt-6 flex items-center gap-3 rounded-sm border p-4 ${notaColor(article.nota)}`}>
            <Star width={22} height={22} className="shrink-0" />
            <div>
              <p className="text-xs uppercase tracking-wide text-ink-dim">Nota final</p>
              <p className="font-display text-2xl font-bold uppercase">{article.nota}/10</p>
            </div>
          </div>
        )}
