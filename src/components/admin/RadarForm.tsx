...
export function RadarForm({ articleId }: RadarFormProps) {
  ...
  async function handleSave() {
    ...
    const result = articleId
      ? await supabase.from("radar_articles").update(payload).eq("id", articleId).select("id")
      : await supabase.from("radar_articles").insert(payload).select("id").single();
    ...
    if (!articleId && result.data && "id" in (result.data as object)) {
      router.push(`/admin/radar/${(result.data as { id: string }).id}`);
    } else {
      router.refresh();
    }
  }
  ...
  async function handleDelete() {
    ...
    const supabase = createBrowserSupabaseClient();
    const { error: delError } = await supabase.from("radar_articles").delete().eq("id", articleId);
    if (delError) {
    ...
    <div>
      <div className="mb-3 flex items-center justify-between">
        <h1 className="font-display text-2xl font-bold uppercase tracking-wide text-ink">
          {articleId ? `Editar: ${form.title || "..."}` : "Novo — Radar+"}
        </h1>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="flex flex-col gap-1.5 sm:col-span-2">
            ...
            <input
              ...
              placeholder="ex: Anunciado o próximo God of War"
          ...
          <label className="flex flex-col gap-1.5">
            <span className={labelClass}>Plataforma (opcional)</span>
            <input
              type="text"
              value={form.platform}
              onChange={(e) => setForm((f) => ({ ...f, platform: e.target.value }))}
              placeholder="ex: PS5, PC"
              className={inputClass}
            />
          </label>
          <label className="flex flex-col gap-1.5 sm:col-span-2">
            <span className={labelClass}>Link do vídeo (YouTube, opcional)</span>
            <input
              type="text"
