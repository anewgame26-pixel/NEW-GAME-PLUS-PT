...
import { getRadarArticleBySlug } from "@/lib/data/radar";
...
import { RadarArticleBody } from "@/components/game/RadarArticleBody";
...
export default async function RadarArtigoPage({ params }: ArtigoPageProps) {
  const { slug } = await params;
  const article = await getRadarArticleBySlug(slug);
  ...
  const [hourWithArticles, retroArticles, discoveryArticles, games] = await Promise.all([
    getHourWithArticles(),
    getRetroArticles(),
  ...
  const matchingRetro = retroArticles.find((a) => normalizeTitle(a.title) === normalizeTitle(article.title));
    ...
    <div className="mt-4 flex flex-col gap-2">
      ...
      {matchingRetro && (
        <CrossLinkBanner
          ...
          icon="history"
          title="Este jogo também está em Retro+"
          description="Vê se ainda vale a pena hoje"
        />
      )}
      {matchingDiscovery && (
        <CrossLinkBanner
  ...
  return (
    <>
      ...
      />
      <Header />
      <GameBreadcrumb items={[{ label: "Radar+", href: "/radar" }, { label: article.title }]} />
      <main>
        <RadarArticleBody article={article} extraSlot={crossLinks} author={author} />
      </main>
      <Footer />
    </>
  );
