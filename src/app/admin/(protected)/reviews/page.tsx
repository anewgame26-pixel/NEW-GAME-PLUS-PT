...
export default async function AdminReviewsPage() {
  const articles = await getAllReviewsAdmin();
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-muted">Análises completas — a review a sério.</p>
        </div>
        <Link
          href="/admin/reviews/novo"
      ...
      {articles.length === 0 ? (
        <p className="py-8 text-sm text-ink-muted">
          Ainda não há reviews. Cria a primeira acima.
        </p>
      ) : (
            ...
            >
              <div className="min-w-0">
                ...
                <p className="text-xs text-ink-dim">
                  ...
                  {article.nota !== null ? ` · Nota: ${article.nota}/10` : ""}
                </p>
              </div>
            </Link>
          ))}
