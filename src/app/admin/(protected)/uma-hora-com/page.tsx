...
import { getAllHourWithArticlesAdmin } from "@/lib/data/hour-with";

export const dynamic = "force-dynamic";

export default async function AdminUmaHoraComPage() {
  const articles = await getAllHourWithArticlesAdmin();
    ...
    <div>
      ...
      {articles.length === 0 ? (
        <p className="py-8 text-sm text-ink-muted">
          Ainda não há artigos. Cria o primeiro acima.
        </p>
      ) : (
        <div className="flex flex-col divide-y divide-border rounded-sm border border-border bg-bg-surface">
          {articles.map((article) => (
            <Link
              key={article.id}
              href={`/admin/uma-hora-com/${article.id}`}
            ...
            >
              <div className="min-w-0">
                ...
                <p className="text-xs text-ink-dim">{article.platform || "Plataforma por preencher"}</p>
              </div>
            </Link>
          ))}
        </div>
