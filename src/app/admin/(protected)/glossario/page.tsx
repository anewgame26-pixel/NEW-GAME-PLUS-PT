...
const emptyForm = { term: "", definition: "", sort_order: 0 };
...
export default function AdminGlossarioPage() {
  ...
  async function loadTerms() {
    ...
    if (error) {
      setError("Não foi possível carregar o glossário. Tenta novamente.");
    } else {
      setTerms(data ?? []);
    }
    setLoading(false);
  ...
  function startEdit(row: GlossaryRow) {
    ...
    setEditingId(row.id);
    setForm({ term: row.term, definition: row.definition, sort_order: row.sort_order });
  }

  function startNew() {
    setEditingId(null);
    setShowNewForm(true);
    const nextOrder = terms.length > 0 ? Math.max(...terms.map((t) => t.sort_order)) + 1 : 1;
  ...
  async function handleSave() {
    ...
    if (editingId) {
      ...
      if (error) setError("Não foi possível criar o termo.");
    }
  ...
  async function handleDelete(id: string) {
    if (!confirm("Apagar este termo do glossário? Não é possível desfazer.")) return;
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-muted">
            Termos que aparecem em {'"'}Glossário de Troféus{'"'} no site público.
          </p>
        </div>
        {!isFormOpen && (
          <button
      ...
      {isFormOpen && (
        <div className="mb-6 rounded-sm border border-border bg-bg-surface p-5">
          <h2 className="mb-4 font-display text-sm font-bold uppercase tracking-wide text-ink-dim">
            {editingId ? "Editar Termo" : "Novo Termo"}
          </h2>
          <div className="flex flex-col gap-4">
            <label className="flex flex-col gap-1.5">
              ...
              <input
                ...
                placeholder="Ex: Missable / Perdível"
            ...
            <label className="flex flex-col gap-1.5">
              ...
              <textarea
                ...
                placeholder="Explica o termo em 1-2 frases..."
      ...
      {loading ? (
        ...
        <p className="py-8 text-sm text-ink-muted">
          Ainda não há termos no glossário. Cria o primeiro acima.
        </p>
      ) : (
          ...
          {terms.map((row) => (
            <div key={row.id} className="flex items-start justify-between gap-4 p-4">
              <div className="min-w-0">
                <p className="font-display text-sm font-bold text-ink">{row.term}</p>
                <p className="mt-1 text-sm text-ink-muted">{row.definition}</p>
              </div>
