...
function mapRowToArticle(row: Record<string, unknown>): RadarArticle {
  return {
    id: row.id as string,
...
export async function getRadarArticles(): Promise<RadarArticle[]> {
  ...
  if (error) {
    console.error("Erro ao carregar artigos Radar+ do Supabase:", error);
    return [];
  }
...
export async function getRadarArticleByGameId(gameId: string): Promise<RadarArticle | null> {
  const { data, error } = await supabase
    .from("radar_articles")
    .select("*")
    .eq("game_id", gameId)
...
export async function getAllRadarArticlesAdmin(): Promise<RadarArticle[]> {
  ...
  if (error) {
    console.error("Erro ao carregar artigos Radar+ (admin) do Supabase:", error);
    return [];
  }
...
export async function getAllRadarSlugs(): Promise<string[]> {
  const { data, error } = await supabase.from("radar_articles").select("slug").eq("is_published", true);
  if (error) return [];
